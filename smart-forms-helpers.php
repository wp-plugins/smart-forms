...
function rednao_smart_forms_load_form($title,$form_id,$returnComponent)
{
    ...
    {
        global $wpdb;
        $result=$wpdb->get_results($wpdb->prepare("select form_id,element_options,form_options from ".SMART_FORMS_TABLE_NAME." where form_id=%d",$form_id));
        if(count($result)>0)
        {
            $result=$result[0];
    ...
    if($returnComponent==false)
    {
        ...
        <script>
            ...
            window.smartFormsItemsToLoad.push({'form_id':<?php echo $options['form_id']?>, 'elements':<?php echo $options['elements']?>,'container':'formContainer<?php echo $random?>'});
        ...
        return "<div id='formContainer$random' class='rednaoFormContainer'></div>
            <script>
                ...
                if(!window.smartFormsItemsToLoad)
                    window.smartFormsItemsToLoad=new Array();;
                window.smartFormsItemsToLoad.push({ 'form_id':".$options['form_id'].",  'elements':".$options['elements'].",'container':'formContainer$random'});
            </script>
           ";
    }
}